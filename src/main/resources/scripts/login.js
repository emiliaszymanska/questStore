...
const color1 = '#707070';
const gradient = 'L(0, 0, 300, 300)#202020';
const maskOffset = btn.clientHeight;

const border = svg.
rect(0, 0, width, height).
attr({ 'fill': 'none', 'stroke': color1, 'stroke-width': 4, 'class': 'border', 'stroke-linecap': 'round'});

const mask = svg.
path(`M0,0 L${width + maskOffset},0 L${width},${height} L-${maskOffset}, ${height} Z`).
attr({ 'fill': 'white', 'stroke': 'white', 'class': 'mask', 'stroke-width':  2}).
...
const button = svg.
group(text, border).
attr({ mask: maskInvert,  'stroke-width': 4});
